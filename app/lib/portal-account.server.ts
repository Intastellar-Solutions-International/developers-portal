...
async function loadPortalAccountFromRequest(
  ...
  /**
   * No `inta_portal_session` and no migratable legacy cookie: the SDK may still show a user,
   * but the portal session is only created via `POST /auth/session` with `user` (client).
   */
  return { account: null, setCookieHeaders };
}

/** Root loader — reads signed session cookie (and migrates legacy cookies once). */
export async function resolvePortalSessionForRequest(
  request: Request,
): Promise<PortalSessionResolution> {
  return loadPortalAccountFromRequest(request);
}
