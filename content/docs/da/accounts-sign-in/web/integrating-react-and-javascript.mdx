...
**Log ud (global API):** Intastellar tilbyder **`Intastellar.accounts.signout()`**, som fjerner cookien **`inta_account`** på din origin. Brug **`logout`** fra **`useIntastellar`** i React-apps; på vanilla-sider med det globale accounts-API, kald **`signout()`**, når scriptet er indlæst.
