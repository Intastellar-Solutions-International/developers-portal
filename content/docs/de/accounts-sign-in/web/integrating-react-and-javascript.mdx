...
**Abmeldung (globale API):** Intastellar stellt **`Intastellar.accounts.signout()`** bereit; damit wird das **`inta_account`**-Cookie auf Ihrer Origin entfernt. In React-Apps **`logout`** aus **`useIntastellar`** nutzen; auf Vanilla-Seiten mit globalem Accounts-API nach dem Laden des Skripts **`signout()`** aufrufen.
