...
Nutzen Sie bevorzugt Intastellars Abmelde-API, damit das **`inta_account`**-Cookie wie in Produktion entfernt wird:

```javascript
Intastellar.accounts.signout();
// Anschließend weiterleiten oder neu laden, z. B.:
// window.location.href = "/";
```

Wenn die API nicht verfügbar ist (z. B. Skript nicht geladen), können Sie das Cookie manuell für Ihre Parent-Domain löschen — **Name** sowie **Domain/Pfad** müssen Ihrer Integration entsprechen (siehe SDK-Readme):
