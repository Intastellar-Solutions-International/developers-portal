...
5. Remove your listener, call `IntastellarAPI.verifyToken(token)`, then persist the session. The React hook sets a first-party cookie:
   ...
   To clear Intastellar’s browser session from your origin, call **`Intastellar.accounts.signout()`** (it removes the **`inta_account`** cookie). See [Logout, errors, and troubleshooting](/docs/accounts-sign-in/web/logout-and-errors).
...
Prefer Intastellar’s sign-out API so the **`inta_account`** cookie is removed the same way as in production:

```javascript
Intastellar.accounts.signout();
// Then redirect or reload as your app requires, e.g.:
// window.location.href = "/";
```

If you cannot call the API (e.g. script not loaded), you can clear the cookie manually for your parent domain—match the **name** and **domain/path** your integration uses (see the SDK readme):
