...
**Déconnexion (API globale) :** Intastellar expose **`Intastellar.accounts.signout()`**, qui supprime le cookie **`inta_account`** sur votre origine. Dans les apps React, utilisez **`logout`** de **`useIntastellar`** ; sur les pages vanilla qui chargent le global accounts, appelez **`signout()`** une fois le script disponible.
