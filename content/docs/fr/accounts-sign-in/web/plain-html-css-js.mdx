...
Préférez l’API de déconnexion Intastellar pour supprimer le cookie **`inta_account`** comme en production :

```javascript
Intastellar.accounts.signout();
// Puis redirigez ou rechargez selon votre app, par ex. :
// window.location.href = "/";
```

Si l’API n’est pas disponible (script non chargé), vous pouvez effacer le cookie manuellement sur votre domaine parent — le **nom** et le **domaine/chemin** doivent correspondre à votre intégration (voir le readme du SDK) :
